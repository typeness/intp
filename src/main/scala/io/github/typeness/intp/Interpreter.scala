...
class Interpreter extends ASTVisitor {
  ...
  private def booleanOperands(left: Boolean, op: Token, right: Boolean): Any = op.tokenType match {
    case AND => left && right
    case OR => left || right
    case EQUALS => left == right
    case NOT_EQUALS => left != right
  ...
  private def charOperands(left: Char, op: Token, right: Char): Any = op.tokenType match {
    case EQUALS => left == right
    case NOT_EQUALS => left != right
    case _ => throw WrongBinaryOperator(left, op, right, compilationUnit, op.position)
  }

  private def arrayOperands(left: mutable.ArrayBuffer[_], op: Token, right: mutable.ArrayBuffer[_]): Any = op.tokenType match {
    case PLUS => left ++ right
    case EQUALS => left == right
    case NOT_EQUALS => left != right
    case _ => throw WrongBinaryOperator(s"[${left.mkString(", ")}]", op, s"[${right.mkString(", ")}]", compilationUnit, op.position)
  }
  ...
  override protected def binOp(ast: BinOp): Any = (visit(ast.left), visit(ast.right)) match {
    ...
    case (left, right) => throw WrongBinaryOperator(left, ast.op, right, compilationUnit, ast.op.position)
  }
  ...
  override protected def unaryOp(ast: UnaryOp): Any = visit(ast.expr) match {
    case value: Double => ast.op.tokenType match {
      case PLUS => value
      case MINUS => -value
      case _ => throw WrongUnaryOperator(ast.op, ast.expr.token.value, compilationUnit, ast.expr.token.position)
    }
    case value: Int => ast.op.tokenType match {
      case PLUS => value
      case MINUS => -value
      case _ => throw WrongUnaryOperator(ast.op, ast.expr.token.value, compilationUnit, ast.expr.token.position)
    }
  ...
  override protected def arrayAssignAST(ast: ArrayAssignAST): Any = visit(ast.source) match {
    case arr: mutable.ArrayBuffer[_] =>
      visit(ast.index) match {
        ...
        case value => throw TypeMismatch(value, IntegerType, compilationUnit, ast.source.token.position)
      }
    case value => throw TypeMismatch(value, ArrayType, compilationUnit, ast.source.token.position)
  }

  override protected def varAST(ast: VarAST): Any = memory.get(ast.name.value) match {
    case Some(variable) => variable
    case None => throw UndefinedVariable(ast.name.value, compilationUnit, ast.name.position)
  }
  ...
  override protected def functionCall(ast: FunctionCall): Any = visit(ast.source) match {
    case FunctionLiteral(formalParameters, body, _) =>
      if (formalParameters.size != ast.actualParameters.size) {
        ...
        throw WrongFunctionCall(fnName, ast.actualParameters.size, formalParameters.size, compilationUnit, ast.source.token.position)
      }
      ...
      memory.popStack()
      result
    case value => throw TypeMismatch(value, FunctionType, compilationUnit, ast.source.token.position)
  }
